...
async function getChats() {
  ...
  const rows = data?.results || data?.data || data?.chats || (Array.isArray(data) ? data : []);
  return (rows || []).map((r) => ({
...
async function getChatMessages(jid, limit = 50) {
  const data = await gowaFetch(`/chat/${encodeURIComponent(jid)}/messages?limit=${limit}`);
  const rows = data?.results || data?.data || data?.messages || (Array.isArray(data) ? data : []);
  return (rows || []).map((m) => ({
