...
async function updateClient(id, p) {
  ...
  const fieldMap = {
    ...
    Notes: 'notes', BulkBilling: 'bulk_billing', ImageUrl: 'image_url',
  };
...
// ─── Notes (Notes tab) ─────────────────────────────────────────────────
// Freestanding text notes with a status, not tied to any client.
...
async function getNotes() {
  ...
  if (error) fail(error, 'Failed to load notes');
  return data.map(toNoteJSON);
}

async function addNote(p) {
  const text = (p.text || '').trim();
  if (!text) fail(null, 'Note text is required');
  ...
  const { data, error } = await supabase
    ...
    .select()
    .single();
  if (error) fail(error, 'Failed to add note');
  return toNoteJSON(data);
}

async function updateNote(noteId, p) {
  const patch = { updated_at: new Date().toISOString() };
  if (p.text !== undefined) {
    const text = (p.text || '').trim();
    if (!text) fail(null, 'Note text is required');
  ...
  const { data, error } = await supabase
    ...
    .select()
    .single();
  if (error) fail(error, 'Note not found: ' + noteId);
  return toNoteJSON(data);
}

async function deleteNote(noteId) {
  const { error } = await supabase.from('ga_notes').delete().eq('id', noteId);
  if (error) fail(error, 'Note not found: ' + noteId);
...
// The list of numbers the Messages tab should show. Empty = unfiltered
// (every GoWA chat is shown). Numbers are stored digits-only so they can
// be compared directly against gowa.jidToPhone(chat.jid).

function toWhatsappNumberJSON(row) {
  if (!row) return null;
  return {
    Id: row.id,
    Number: row.number,
...
async function getWhatsappNumbers() {
  const { data, error } = await supabase
    .from('ga_whatsapp_numbers')
    .select('*')
    .order('created_at', { ascending: true });
  if (error) fail(error, 'Failed to load WhatsApp number filter');
  return data.map(toWhatsappNumberJSON);
}

async function addWhatsappNumber(p) {
  const number = normalizeNumber(p && p.number);
  if (!number) fail(null, 'A phone number is required');

  const { data, error } = await supabase
    .from('ga_whatsapp_numbers')
    .upsert({ number, label: (p.label || '').trim() || null }, { onConflict: 'number' })
    .select()
    .single();
  if (error) fail(error, 'Failed to add number');
  return toWhatsappNumberJSON(data);
}

async function deleteWhatsappNumber(id) {
  const { error } = await supabase.from('ga_whatsapp_numbers').delete().eq('id', id);
  if (error) fail(error, 'Number not found: ' + id);
  return { success: true, id };
}
...
module.exports = {
  ...
  getWhatsappNumbers, addWhatsappNumber, deleteWhatsappNumber,
